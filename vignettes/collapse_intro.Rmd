...
* *collapse* provides an organized and comprehensive suite of advanced statistical operations such as grouped, weighted and groupwise weighted statistics, grouped sweeping of statistics, scaling, centering, higher-dimensional centering and linear prediction, panel-data methods and summary statistics, and various time-series operations such as iterated-, panel-, quasi-, log- differences and growth rates.

* *collapse* is extremely fast *and* micro-optimized. Many grouped and weighted computations are noticeably faster than doing them in *dplyr* or *data.table*. The standard execution speed of *collapse* functions on small data is between 10 and 100 microseconds. This speed becomes noticeable when executing a page of code, writing a statistical program or the server code for a *shiny* app.
...
* *collapse* is broadly object oriented and not centered around data.frames. It supports all R data structures, in particular vectors / time-series / panel-series, matrices, data.frames / grouped data.frames (`dplyr::grouped_df`) / panel data.frames (`plm::pdata.frame`) and general lists of data objects. 
...
Doing this has only one advantage on my opinion and that is that we can use `unlist2d` to transform this list into a tidy data.frame (more about this in the section on list-processing):
...
As a final step we consider a plot function which can be used to plot the structural transformation of any supported country. Below for Tanzania:
```{r}
library(data.table)
library(ggplot2)
...
Using *collapse*'s fast functions and the programming principles laid out here can speed up grouped computations by orders of magnitude - even compared to packages like *dplyr* or *data.table*. Simple column-wise computations on matrices are also slightly faster than with `base` functions like `colMeans`, `colSums` 
...
*collapse* will soon support faster interaction with the help of algorithms provided by the *fixest* R package. For the moment this feature is still rather experimental. 
...
Note that all of this does not require the panel to be balanced. `flag / L /F` works for unbalanced panel data as long as there are no gaps or repeated values in the time-variable for an individual. In such cases, `flag / L /F` can still be used, but one needs to call the function `seqid` first to generate an appropriate Id variable which identifies consecutive time-sequences at the sub-individual level.  One intended area of use, especially for the operators `L` and `F`, is to dramatically facilitate the implementation of dynamic models in various contexts. Below different ways `L` can be used to estimate a dynamic panel-model using `lm` are shown:
```{r}
