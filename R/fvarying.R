...
varying.default <- function(x, g = NULL, any_group = TRUE, use.g.names = TRUE, ...) {
  ...
  if(is.null(g)) return(.Call(Cpp_varying,x,0L,0L,any_group)) else if(is.atomic(g)) {
    ...
    if(use.g.names && !any_group) return(`names<-`(.Call(Cpp_varying,x,g[[1L]],g[[2L]],any_group), group_names.GRP(g))) else
...
varying.pseries <- function(x, effect = 1L, any_group = TRUE, use.g.names = TRUE, ...) {
  ...
  if(!any_group && use.g.names) {
    lev <- attr(g, "levels")
    return(`names<-`(.Call(Cpp_varying,x,length(lev),g,any_group), lev))
  } else .Call(Cpp_varying,x,fnlevels(g),g,any_group)
}
...
varying.matrix <- function(x, g = NULL, any_group = TRUE, use.g.names = TRUE, drop = TRUE, ...) {
  ...
  if(is.null(g)) return(.Call(Cpp_varyingm,x,0L,0L,any_group,drop)) else if(is.atomic(g)) {
    ...
    if(!is.GRP(g)) g <- if(use.g.names && !any_group) GRP.default(g) else GRP.default(g, return.groups = FALSE)
    if(use.g.names && !any_group) return(`dimnames<-`(.Call(Cpp_varyingm,x,g[[1L]],g[[2L]],any_group,FALSE), list(group_names.GRP(g), dimnames(x)[[2L]]))) else
...
varying.data.frame <- function(x, by = NULL, cols = NULL, any_group = TRUE, use.g.names = TRUE, drop = TRUE, ...) {
  ...
  if(is.call(by)) {
    ...
    by <- if(length(gn) == 1L) .subset2(x, gn) else GRP.default(x, gn)
    x <- fcolsubset(x, cols)
  } else if(!is.null(cols)) x <- colsubset(x, cols)

  if(is.null(by)) return(.Call(Cpp_varyingl,x,0L,0L,any_group,drop)) else if(is.atomic(by)) {
    if(use.g.names && !any_group && !inherits(x, "data.table")) {
      if(!is.nmfactor(by)) by <- qF(by, na.exclude = FALSE)
      lev <- attr(by, "levels")
      return(setRow.names(.Call(Cpp_varyingl,x,length(lev),by,any_group,FALSE), lev))
    } else {
      if(is.nmfactor(by)) return(.Call(Cpp_varyingl,x,fnlevels(by),by,any_group,drop)) else {
        by <- qG(by, na.exclude = FALSE)
        return(.Call(Cpp_varyingl,x,attr(by,"N.groups"),by,any_group,drop))
      }
    }
  } else {
    if(!is.GRP(by)) by <- if(use.g.names && !any_group) GRP.default(by) else GRP.default(by, return.groups = FALSE)
    if(use.g.names && !any_group && !inherits(x, "data.table") && !is.null(groups <- group_names.GRP(by)))
      return(setRow.names(.Call(Cpp_varyingl,x,by[[1L]],by[[2L]],any_group,FALSE), groups)) else
        return(.Call(Cpp_varyingl,x,by[[1L]],by[[2L]],any_group,drop))
  }
}
...
varying.pdata.frame <- function(x, effect = 1L, cols = NULL, any_group = TRUE, use.g.names = TRUE, drop = TRUE, ...) {
  ...
  if(!any_group && use.g.names) {
    lev <- attr(g, "levels")
    return(setRow.names(.Call(Cpp_varyingl,x,length(lev),g,any_group,FALSE), lev))
...
#         return(setRow.names(.Call(Cpp_varyingl,x,g[[1L]],g[[2L]],any_group,FALSE), groups)) else
#           return(.Call(Cpp_varyingl,x,g[[1L]],g[[2L]],any_group,drop))
#     }
#   } else {
