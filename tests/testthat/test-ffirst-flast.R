...
test_that("ffirst produces errors for wrong input", {
  ...
  expect_error(ffirst("a", w = 1))
  expect_error(ffirst(1:2, w = 1:3))
  expect_error(ffirst(NA_character_, w = 1))
  expect_error(ffirst(mNA, w = wdat))
  expect_error(ffirst(mNA, f, 2))
  expect_error(ffirst(mNA, w = 1:33))
  expect_error(ffirst(1:2,1:2, 1:3))
  expect_error(ffirst(m,1:32,1:20))
  expect_error(ffirst(data,1:32,1:10))
  expect_error(ffirst(1:2, w = c("a","b")))
  expect_visible(ffirst(wlddev))
  expect_error(ffirst(wlddev, w = wlddev$year, drop = FALSE))
...
test_that("flast produces errors for wrong input", {
  ...
  expect_error(flast("a", w = 1))
  expect_error(flast(1:2, w = 1:3))
  expect_error(flast(NA_character_, w = 1))
  expect_error(flast(mNA, w = wdat))
  expect_error(flast(mNA, f, wdat))
  expect_error(flast(mNA, w = 1:33))
  expect_error(flast(1:2,1:2, 1:3))
  expect_error(flast(m,1:32,1:20))
  expect_error(flast(data,1:32,1:10))
  expect_error(flast(1:2, w = c("a","b")))
  expect_visible(flast(wlddev))
  expect_error(flast(wlddev, w = wlddev$year, drop = FALSE))
