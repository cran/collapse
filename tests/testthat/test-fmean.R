...
test_that("fmean with weights performs like stats::weighted.mean", {
  # complete weights
  expect_equal(fmean(NA, w = 1), weighted.mean(NA, 1))
  expect_equal(fmean(NA, w = 1, na.rm = FALSE), weighted.mean(NA, 1))
  expect_equal(fmean(1, w = 1), weighted.mean(1, w = 1))
  expect_equal(fmean(1:3, w = 1:3), weighted.mean(1:3, 1:3))
  expect_equal(fmean(-1:1, w = 1:3), weighted.mean(-1:1, 1:3))
  expect_equal(fmean(1, w = 1, na.rm = FALSE), weighted.mean(1, 1))
  expect_equal(fmean(1:3, w = c(0.99,3454,1.111), na.rm = FALSE), weighted.mean(1:3, c(0.99,3454,1.111)))
  expect_equal(fmean(-1:1, w = 1:3, na.rm = FALSE), weighted.mean(-1:1, 1:3))
  expect_equal(fmean(x, w = w), weighted.mean(x, w))
  expect_equal(fmean(x, w = w, na.rm = FALSE), weighted.mean(x, w))
  expect_equal(fmean(xNA, w = w, na.rm = FALSE), weighted.mean(xNA, w))
  expect_equal(fmean(xNA, w = w), weighted.mean(xNA, w, na.rm = TRUE))
  expect_equal(fmean(mtcars, w = wdat), fmean(m, w = wdat))
  expect_equal(fmean(m, w = wdat), dapply(m, weighted.mean, wdat, na.rm = TRUE))
  expect_equal(fmean(m, w = wdat, na.rm = FALSE), dapply(m, weighted.mean, wdat))
  expect_equal(fmean(mNA, w = wdat, na.rm = FALSE), dapply(mNA, weighted.mean, wdat))
  expect_equal(fmean(mNA, w = wdat), dapply(mNA, weighted.mean, wdat, na.rm = TRUE))
  expect_equal(fmean(x, f, w), mapply(weighted.mean, split(x, f), split(w, f)))
  expect_equal(fmean(x, f, w, na.rm = FALSE), mapply(weighted.mean, split(x, f), split(w, f)))
  expect_equal(fmean(xNA, f, w, na.rm = FALSE), mapply(weighted.mean, split(xNA, f), split(w, f)))
  expect_equal(fmean(xNA, f, w), mapply(weighted.mean, split(xNA, f), split(w, f), na.rm = TRUE))
  # missing weights
  expect_equal(fmean(NA, w = NA), weighted.mean(NA, NA))
  expect_equal(fmean(NA, w = NA, na.rm = FALSE), weighted.mean(NA, NA))
  expect_equal(fmean(1, w = NA), weighted.mean(1, w = NA))
  # expect_equal(fmean(1:3, w = c(NA,1:2)), weighted.mean(1:3, c(NA,1:2), na.rm = TRUE)) # weighted mean doesn not remove NA's from weights !!
  # expect_equal(fmean(-1:1, w = c(NA,1:2)), weighted.mean(-1:1, c(NA,1:2), na.rm = TRUE))
  expect_equal(fmean(1, w = NA, na.rm = FALSE), weighted.mean(1, NA))
  expect_equal(fmean(1:3, w = c(NA,1:2), na.rm = FALSE), weighted.mean(1:3, c(NA,1:2)))
  expect_equal(fmean(-1:1, w = c(NA,1:2), na.rm = FALSE), weighted.mean(-1:1, c(NA,1:2)))
  # expect_equal(fmean(x, w = wNA), weighted.mean(x, wNA, na.rm = TRUE))
  expect_equal(fmean(x, w = wNA, na.rm = FALSE), weighted.mean(x, wNA))
  expect_equal(fmean(xNA, w = wNA, na.rm = FALSE), weighted.mean(xNA, wNA))
  # expect_equal(fmean(xNA, w = wNA), weighted.mean(xNA, wNA, na.rm = TRUE))
  expect_equal(fmean(mtcars, w = wdatNA), fmean(m, w = wdatNA))
  # expect_equal(fmean(m, w = wdatNA), dapply(m, weighted.mean, wdatNA, na.rm = TRUE))
  expect_equal(fmean(m, w = wdatNA, na.rm = FALSE), dapply(m, weighted.mean, wdatNA))
  expect_equal(fmean(mNA, w = wdatNA, na.rm = FALSE), dapply(mNA, weighted.mean, wdatNA))
  # expect_equal(fmean(mNA, w = wdatNA), dapply(mNA, weighted.mean, wdatNA, na.rm = TRUE))
  # expect_equal(fmean(x, f, wNA), mapply(weighted.mean, split(x, f), split(wNA, f), na.rm = TRUE))
  expect_equal(fmean(x, f, wNA, na.rm = FALSE), mapply(weighted.mean, split(x, f), split(wNA, f)))
  expect_equal(fmean(xNA, f, wNA, na.rm = FALSE), mapply(weighted.mean, split(xNA, f), split(wNA, f)))
  # expect_equal(fmean(xNA, f, wNA), mapply(weighted.mean, split(xNA, f), split(wNA, f), na.rm = TRUE))
})
