...
test_that("TRA performs like fbetween and fwithin", {
    ...
    expect_equal(TRA(v, fmean(v, f), 4L, f), fwithin(v, f, add.global.mean = TRUE))
    ...
    expect_equal(TRA(m, fmean(m, f), 4L, f), fwithin(m, f, add.global.mean = TRUE))
    ...
    expect_equal(TRA(d, fmean(d, f), 4L, f), fwithin(d, f, add.global.mean = TRUE))
})
...
test_that("TRA gives errors for wrong input", {
  ...
  expect_error(TRA(v, fmean(v), 9L))
  expect_error(TRA(m, fmean(m), 9L))
  expect_error(TRA(d, fmean(d), 9L))
